...
Similarly, the outer projection in the query \ensuremath{\vQ_6} written over 
\ensuremath{\vSch_2} \\
\centerline{
\ensuremath{
\vQ_6 =
...
%
This shows that the type system applies the schema to the type of a query although it does not apply it to the query\footnote{We proved this theorem in the Coq proof assistant. The encoding of the theorem and the proof can be fine here: \TODO{update later. you may have to upload extra files instead of providing a link if submission is double-blind.}}.
\end{theorem}
