...
\rewrite{change intro to what you have on the whiteboard}
\eric{read from here pls.}
In this section, we define how variation is encoded at the schema level. We first 
embark on an example that 
Remember as mentioned in \secref{rdb}, the schema of a database is basically
...
%
For example, consider the \empbio\ relation associated with variant \vThree\ in \tabref{mot-basic},
\structure{make sure this is below in final version}
also shown in \tabref{empbio-v3} below.
% 
However, this relation changes in variants that enable \vFour\ or \vFive, as shown in 
\tabref{empbio-v4} and \tabref{empbio-v5}, respectively.
%
The variational relation schema of \empbio\ captures these differences (variation)
in \tabref{empbio-vsch} by assigning (annotating) a feature expression that 
indicates the variants that \empbio\ and each of its attributes exists in. 
This table can be interpreted as \empbio\ is present in variants that enable 
at least one of \vThree, \vFour, and \vFive. Then, the three attributes 
\empno, \sex, and \birthdate\ exists in the relation \empbio\ no matter what as long as
\empbio\ exists in a variant. However, the \name\ attribute is only present in variants
...
that is, we assume a countably infinite set of \attNames\ of attributes is fixed; we assume
there is a total order $\leq_\attNames$ on \attNames; for any set of attributes \vAttList,
we assume that elements of \vAttList are written according to $\leq_\attNames$; for 
theoretical developments we assume the same domain of values for all attributes; 
we assume a countably infinite set \relNames\ of relation names disjoint from \attNames.
%
\eric{stop reading here. the rest is from the vldb paper. there's another box in this section awaiting your feedback.}
...
\eric{added reference to figure and examples.}
...
\eric{added configuration example.}
