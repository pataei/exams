...
\figref{vsch} formally defines a variational schema. 
...
\exref{vsch} defines a variational schema for only a part of our motivating example (\secref{mot}) while
 \exref{vsch-mot} provides the variational schema of our motivating example. 
...
We define the configuration function for variational schemas and its elements in \figref{vdb-conf} in \appref{vdb-conf}.
...
\tabref{mot} includes all its database variants. 
\input{tables/motVsch}
So, for example, configuring the \wrrite{mot vsch for v2 t3 edu results in the yellow highlighted cells of table mot}.
\end{example}
