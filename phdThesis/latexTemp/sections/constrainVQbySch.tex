...
\wrrite{add that query must be explicitly annotated before being configured}
\maybeAdd{if have time, give the exact query after explicitly annotating them instead of simplifying them.}
...
%
\eric{added this paragraph. pls review it}
\wrrite{point that query passed to config ... has to be explicitly annotated.}
