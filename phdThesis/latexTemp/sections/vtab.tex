...
variants of the relation \empbio\ shown in \tabref{empbio-v3}--\tabref{empbio-v5}.
\tabref{empbio-vsch-comp} illustrates the variational relation \empbio\ but it does not include
variation at the content level.
% the corresponding variational 
%table  shown in \tabref{empbio-sch}. 
%\tabref{empbio-tab}
...
%
Note that the white spaces in \tabref{empbio-vtab} indicate the non-existing values
for an attribute in a tuple. For example, consider the first three tuples of the variational
table \empbio\ shown in \tabref{empbio-vtab}. Since none of the attributes \name, \fname, and \lname\
...
in \tabref{empbio-v3}, assuming the VDB has the variational schema $\vSch_\mot$
...
%As shown, o
Our VDB framework encodes variation 
%in databases 
%\maybeAdd{if time allows give this example over the \tabref{empbio-vtab}.}
